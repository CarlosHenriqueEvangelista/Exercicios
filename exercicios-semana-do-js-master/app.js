...
const IN = randomNumbers.filter(n => n%2 != 0)
...
const crazyNumbers = [937, 8, 395, 402, 501, 333, 502, 781, 3, 691]

const Me501 = crazyNumbers.reduce((acc , crazyNumber) => {
  if (crazyNumber < 501) {
    acc++
  }
  return acc
...
const F2000 = tarantinoMovies.filter( Movies=> {
...
const Names = tvShows.map(n => n.name)
...
const Joojs = cart.reduce((acc , { name }) => 
   `${acc}- ${name}\n `
, '')
console.log(Joojs) 
